"""
COMPLETE PERFECT TRAINING - One Script to Rule Them All
1. Trains perfect binary classifier
2. Updates defect detection
3. Tests the system
"""
import os
import sys
...
def main():
    print("=" * 70)
    print("COMPLETE PERFECT TRAINING - All-in-One Process")
    print("=" * 70)
    
    # Step 1: Train the model
    print("\n[STEP 1/3] Training perfect binary classifier...")
    print("-" * 70)
    try:
    ...
    # Step 2: Update defect detection to use the trained model properly
    print("\n[STEP 2/3] Updating defect detection for perfect accuracy...")
    print("-" * 70)
...
'''
    ...
    try:
        ...
        print(f"ERROR: Failed to update defect_detection.py: {e}")
        return False
    
    # Step 3: Test the system
    print("\n[STEP 3/3] Testing the system...")
    print("-" * 70)
...
def test_system():
    ...
    # Test with a normal image if available
    normal_dir = "data/training/normal"
    if os.path.exists(normal_dir):
        normal_files = [f for f in os.listdir(normal_dir) 
                       if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
        if normal_files:
            test_image = Image.open(os.path.join(normal_dir, normal_files[0]))
            defects = detector.detect(test_image)
            print(f"\\nTest Normal Image: {normal_files[0]}")
            print(f"Detected Defects: {len(defects)}")
            if len(defects) == 0:
                print("✓ PASS: Normal image correctly identified as normal (no defects)")
            else:
                print(f"✗ FAIL: Found {len(defects)} false positives")
                for d in defects:
                    print(f"  - {d['type']}: {d['confidence']:.2f}")
    
    # Test with a defective image if available
    defective_dir = "data/training/defective"
    if os.path.exists(defective_dir):
        defective_files = [f for f in os.listdir(defective_dir) 
                          if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
        if defective_files:
            test_image = Image.open(os.path.join(defective_dir, defective_files[0]))
            defects = detector.detect(test_image)
            print(f"\\nTest Defective Image: {defective_files[0]}")
            print(f"Detected Defects: {len(defects)}")
            if len(defects) > 0:
                print("✓ PASS: Defective image correctly identified")
                for d in defects:
                    print(f"  - {d['type']}: {d['confidence']:.2f} ({d['severity']})")
            else:
                print("✗ FAIL: Defective image not detected")
...
'''
    ...
    try:
        with open('test_perfect_system.py', 'w', encoding='utf-8') as f:
            f.write(test_code)
        
        # Run the test
        result = subprocess.run([sys.executable, 'test_perfect_system.py'], 
    ...
    print("\n" + "=" * 70)
    print("COMPLETE PERFECT TRAINING FINISHED!")
    print("=" * 70)
